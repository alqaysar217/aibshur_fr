import { Search, MapPin, Bell, ChevronLeft, Star } from "lucide-react"
...
const stores = [
  {
    ...
    image: PlaceHolderImages.find(img => img.id === "burger-store")?.imageUrl,
    category: "مطاعم",
  ...
  },
  {
    ...
    image: PlaceHolderImages.find(img => img.id === "pizza-store")?.imageUrl,
    category: "مطاعم",
    tags: ["بيتزا", "ايطالي"]
  },
  {
    ...
    image: PlaceHolderImages.find(img => img.id === "coffee-store")?.imageUrl,
    category: "كافيه",
    tags: ["قهوة", "مشروبات"]
  }
]
    ...
    <div className="pb-20">
      ...
      <header className="p-4 flex items-center justify-between sticky top-0 glass z-40">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm font-bold flex items-center gap-1">المنزل، الرياض <ChevronLeft className="h-3 w-3 rotate-270" /></p>
          </div>
        </div>
      ...
      <section className="p-4 space-y-4">
        <div className="relative group">
          ...
          <Input 
            placeholder="ابحث عن وجبة أو متجر..." 
            className="pr-12 h-14 rounded-2xl border-none shadow-sm bg-white text-base focus-visible:ring-accent"
          />
        </div>

        {/* Banner */}
        <div className="relative h-44 w-full rounded-3xl overflow-hidden shadow-lg">
          {heroImage && (
            <Image 
          ...
          <div className="absolute inset-0 bg-gradient-to-l from-primary/80 to-transparent p-6 flex flex-col justify-center text-white">
            <Badge className="w-fit mb-2 bg-accent text-accent-foreground font-bold">عرض خاص</Badge>
      ...
      <section className="p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/categories" className="text-primary text-sm font-medium">عرض الكل</Link>
        </div>
        <div className="flex gap-4 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
          {categories.map((cat) => (
            <div key={cat.id} className="flex flex-col items-center gap-2 min-w-[70px]">
              <div className="h-16 w-16 bg-white rounded-2xl flex items-center justify-center text-3xl shadow-sm border border-border hover:border-accent transition-colors">
                {cat.icon}
              </div>
      ...
      <section className="p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/stores" className="text-primary text-sm font-medium">عرض الكل</Link>
        </div>
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-4 bg-white">
                    ...
                    <div className="flex gap-2">
                      {store.tags.map(tag => (
                        <Badge key={tag} variant="secondary" className="font-normal text-[10px] bg-secondary/50">{tag}</Badge>
                      ))}
                    </div>
                  </div>
                </CardContent>
